...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    ...
    # Use the inverse error function to calculate the scaled CDF
    with torch.no_grad():
        tensor.normal_(mean=0, std=1)  # Fill the tensor with values from a standard normal distribution
        tensor = tensor.clamp_(min=lower_bound, max=upper_bound)
        tensor.mul_(std).add_(mean)  # Scale and shift the tensor to have the desired mean and std
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.shift_size > 0:
            ...
            attn_mask = mask_matrix.to(shifted_x.dtype)
        else:
            shifted_x = x
            attn_mask = None
...
def build_swint_backbone(frozen_stage):
    """
