...
if [ "$#" -ne 7 ]; then
    echo "Usage: $0 <model_path> <question_path> <base_answer_path> <image_folder> <N> <temperature> <version>"
    exit 1
fi
...
do
    ...
    CUDA_VISIBLE_DEVICES="$chunk_id" python gllava/eval/model_vqa.py --model-path "$model_path" --question-file "$question_path" --answers-file "$answer_path" --num-chunks "$N" --chunk-idx "$chunk_id" --image-folder "$image_folder" --temperature "$temperature" --conv-mode "$conv_mode" &
